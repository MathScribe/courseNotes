function reproduce_paper_fig1_bw_v6()
    ...
    % Use software OpenGL in batch mode when available.
    try
        opengl software;
    catch
    end
    ...
    % Use the v5 framing but remove the excess canvas after export.
    camzoom(1.45);
    ...
    rawFile = fullfile(thisDir,'paper_fig1_reproduction_bw_v6_raw.png');
    outFile = fullfile(thisDir,'paper_fig1_reproduction_bw_v6.png');

    print(gcf,rawFile,'-dpng','-r300');

    rgb = imread(rawFile);
    contentMask = any(rgb < 248,3);
    ...
    if ~isempty(rows) && ~isempty(cols)
        ...
        r2 = min(max(rows)+pad,size(rgb,1));
        c1 = max(min(cols)-pad,1);
        c2 = min(max(cols)+pad,size(rgb,2));
        rgb = rgb(r1:r2,c1:c2,:);
    end
